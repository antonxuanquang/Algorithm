...
public class Twelve {
	...
	private static class Position {
		...
		public boolean isIntersect(Position point) {
			if (row == point.row) return true;
			int columnDistance = Math.abs(column - point.column);
			int rowDistance = Math.abs(row - point.row);
			return columnDistance == rowDistance ? true : false; 
		}
