...
public class OmegaNetwork {
	...
	public static ArrayList<String> permutationNonBlocking(String string,
			String original) {
		ArrayList<String> result = new ArrayList<String>();
		if (string.length() == 0) {
			result.add("");
			...
			for (int i = 0; i < string.length(); i++) {
				char firstChar = string.charAt(i);
				if (firstChar != original.charAt(original.length()
						- string.length())) {
					String subString = string.substring(0, i)
							+ string.substring(i + 1, string.length());
					ArrayList<String> permutations = permutationNonBlocking(
							subString, original);
					for (String sub : permutations) {
						result.add(firstChar + sub);
					}
	...
	public static void main(String args[]) {
		...
		ArrayList<String> permutation = permutation(original);
		// for (String string: permutation) System.out.println(string);
		System.out.println(permutation.size());
		permutation = permutationNonBlocking(original, original);
		// for (String string: permutation) System.out.println(string);
		System.out.println(permutation.size());
	}
}
